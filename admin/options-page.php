...
class Prompt_Admin_Options_Page extends scbAdminPage {
	...
	public function page_head() {
		...
		$script = new Prompt_Script( array(
			...
			'dependencies' => array( 'jquery-ui-tabs', 'datetimepicker' ),
		) );
		$script->enqueue();

		$script->localize( 'prompt_options_page_env', array(
			'email_header_image_prompt' => __( 'Choose an email header image', 'Postmatic' ),
		) );
	}
	...
	protected function add_tabs() {
		...
		$tabs = array(
			...
			new Prompt_Admin_Invite_Options_Tab( $this->options, $this->_overridden_options ),
